...
public class PersonnelAdapter extends BaseAdapter {
	...
	private ArrayList<UserInfo> mFollowList;
	private DisplayImageOptions options;
	private HashMap<String, Integer> postionsMap;
	private int type;
	...
	public HashMap<String, Integer> getPostionMap() {
		return postionsMap;
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			vh.image = (ImageView) convertView.findViewById(R.id.iv_guanzhude);
		...
		if (type == 0) {
			if (position > 0
					&& mDataList.get(position - 1).DeptID == user.DeptID) {
				vh.deptTv.setVisibility(View.GONE);
			} else {
				vh.deptTv.setVisibility(View.VISIBLE);
			}
			vh.deptTv.setText(TextUtils.isEmpty(user.DeptName) ? "其他"
					: user.DeptName);
		}
		if (user.DeptName.equals("我关注的")) {
		...
		convertView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.putExtra("simple_user",
						(UserInfo) v.getTag(R.id.tag_second));
