...
public class FileDownloader {
	...
	static class DownloadHandler extends Handler {
		...
		public void dispatchMessage(Message message) {
			...
			case STATUS_RUNING:
				...
				downloadCallBack.progress(threadKey,
						FILE_SIZE_MAP.get(threadKey),
						PROGRESS_MAP.get(threadKey));

				message = this.obtainMessage();
				message.getData().putString("threadKey", threadKey);
				if (FILE_SIZE_MAP.get(threadKey).equals(
						PROGRESS_MAP.get(threadKey))
	...
	public static interface FileDownloadCallBack {
		public abstract void progress(String threadKey, long fileSize,
				long downloadSize);
