...
public class SwipeBackLayout extends FrameLayout {
    ...
    public static final int EDGE_TOP_BOTTOM = EDGE_TOP | EDGE_BOTTOM;

    /**
     * Edge flag set indicating all edges should be affected.
     */
//    public static final int EDGE_ALL = EDGE_LEFT | EDGE_RIGHT | EDGE_TOP | EDGE_BOTTOM;
    public static final int EDGE_ALL = EDGE_TOP | EDGE_BOTTOM;
    ...
    private static final int[] EDGE_FLAGS = {
            EDGE_LEFT, EDGE_RIGHT, EDGE_TOP, EDGE_BOTTOM, EDGE_ALL
    };
...
//    private Drawable mShadowTop;
    ...
    public SwipeBackLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        int shadowTop = a.getResourceId(R.styleable.SwipeBackLayout_shadow_bottom,
...
//        setShadow(shadowTop, EDGE_TOP);
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        /* 解决在顶部/底部可滑动退出区域能向上/向下滑动视图*/
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            oldY = event.getY();
        } else if (event.getAction() == MotionEvent.ACTION_MOVE) {
            Log.w("action", "oldy:"+oldY+", y:"+event.getY()+", mEdgeFlag:"+mEdgeFlag);
            if (oldY > event.getY() && mEdgeFlag == EDGE_TOP || oldY < event.getY() && mEdgeFlag == EDGE_BOTTOM) {
                return false;
            }
        }
...
//        } catch (ArrayIndexOutOfBoundsException e) {
    ...
    private void drawScrim(Canvas canvas, View child) {
        ...
        } else if ((mTrackingEdge & EDGE_TOP) != 0) { //TODO
            canvas.clipRect(child.getLeft(), child.getTop(), getRight(), child.getTop());
    ...
    private void drawShadow(Canvas canvas, View child) {
        ...
        /*if ((mEdgeFlag & EDGE_TOP) != 0) { //TODO
            mShadowTop.setBounds(childRect.left, childRect.top, childRect.right,
                    childRect.top - mShadowTop.getIntrinsicHeight());
