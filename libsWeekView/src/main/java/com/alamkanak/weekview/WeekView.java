...
public class WeekView extends View {
    ...
    private Paint mEventStrokePaint; //事件边框
    ...
    private int mTodayBackgroundColor = Color.rgb(255, 255, 255);
    ...
    private void init() {
        ...
        mEventStrokePaint.setAntiAlias(true);  //消除锯齿
        mEventStrokePaint.setColor(Color.WHITE); //设置边框颜色
        mEventStrokePaint.setStyle(Paint.Style.STROKE);
        mEventStrokePaint.setStrokeWidth(3); //设置边框宽度
    ...
    private void drawEventTitle(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        ...
        switch (colorId) {
            case -1:
                mEventTextColor = R.color.cal_text;
                break;
            case 0:
                mEventTextColor = R.color.cal_text1;
                break;
            case 1:
                mEventTextColor = R.color.cal_text2;
                break;
            case 2:
                mEventTextColor = R.color.cal_text3;
                break;
            case 3:
                mEventTextColor = R.color.cal_text4;
                break;
            case 4:
                mEventTextColor = R.color.cal_text5;
                break;
            case 5:
                mEventTextColor = R.color.cal_text6;
                break;
            case 6:
                mEventTextColor = R.color.cal_text7;
                break;
        }
        mEventTextPaint.setColor(mEventTextColor);
    ...
    private void getMoreEvents(Calendar day) {
        ...
        while (tempEvents.size() > 0) {
            ArrayList<EventRect> eventRects = new ArrayList<EventRect>(tempEvents.size());
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        boolean val = mGestureDetector.onTouchEvent(e);
        ...
        if (e.getAction() == MotionEvent.ACTION_UP && !mIsZooming && mCurrentFlingDirection == Direction.NONE) {
            //            if (mCurrentScrollDirection == Direction.HORIZONTAL) {
            //                goToNearestOrigin();
            //            }
            mCurrentScrollDirection = Direction.NONE;
        }
        //        if (e.getY() < getTop() + dip2px(50) || e.getY() > getBottom() - dip2px(50)) {
        ...
        return val;
        // TODO OnTouchCrash
        //        if (e.getAction() != MotionEvent.ACTION_MOVE && mEventRects != null && mEventClickListener != null) {
        //            List<EventRect> reversedEventRects = mEventRects;
        //            Collections.reverse(reversedEventRects);
        //            for (EventRect event : reversedEventRects) {
        //                if (event.rectF != null && e.getX() > event.rectF.left && e.getX() < event.rectF.right && e.getY() > event.rectF.top && e.getY() < event.rectF.bottom) {
