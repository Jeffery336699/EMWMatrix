...
public class WeekViewEvent implements Comparable<WeekViewEvent>,Serializable {
	...
	public WeekViewEvent() {
		this.mStartDay = mStartTime.get(Calendar.DAY_OF_MONTH);
	}
...
/**
	...
	private int getEventStartDay(Calendar time) {
		return time.get(Calendar.DAY_OF_MONTH);
	}

	@Override
	public int compareTo(WeekViewEvent arg0) {
		if (arg0.mStartTime.getTimeInMillis() - this.mStartTime.getTimeInMillis()>0) {
			return -1;
		}else if (arg0.mStartTime.getTimeInMillis() - this.mStartTime.getTimeInMillis()<0) {
			return 1;
		}else{
			return 0;
		}
