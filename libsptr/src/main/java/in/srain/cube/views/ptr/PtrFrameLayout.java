    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if(toggleMenu) {
                    float nowY = e.getY();
                    float dy = Math.abs(nowY - lastY);
                    Log.d("lcy", "onTouch()->lastY:" + lastY + " nowY: " + nowY);
                    if (dy > ViewConfiguration.getTouchSlop()) {
                        if (lastY > nowY && !isTopBottomToggle) {
                            //发送广播------
                            Log.d("lcy", "onTouch()->send browxcase: ");
                            isTopBottomToggle = true;
                            toggleTopAndBottom("cc.emw.mobile.hide_top_and_bottom");
                        } else if (lastY < nowY && isTopBottomToggle) {
                            isTopBottomToggle = false;
                ...
                if (Math.abs(offsetX) > Math.abs(offsetY)) {
                    //需求：在刷新头可见的时候，拦截分发事件，ptr自己消费
//                    Log.d("ptr", "offsetX:" + offsetX + "..offsetY:" + offsetY);
                    mPreventForHorizontal = true;
                } else {
                    mPreventForHorizontal = false;
                }
    ...
    /**
     * 设置是否触发收缩菜单广播，默认为false
     * @param isToggle
     */
    public void setToggleMenu(boolean isToggle){
        this.toggleMenu = isToggle;
    }
}
