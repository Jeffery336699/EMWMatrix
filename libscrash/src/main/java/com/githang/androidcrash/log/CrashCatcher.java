...
public class CrashCatcher implements UncaughtExceptionHandler {
    ...
    public void uncaughtException(final Thread thread, final Throwable ex) {
        try {
			LogWriter.writeLog(mLogFile, AppManager.currentActivity()
					.getClass().getCanonicalName(), mDeviceInfo, ex);
        }catch (Exception e) {
            Log.w(LOG_TAG, e);
        }
        ex.printStackTrace();
     ...
     *            回调接口
     */
    public void init(File logFile, CrashListener listener, String deviceInfo) {
