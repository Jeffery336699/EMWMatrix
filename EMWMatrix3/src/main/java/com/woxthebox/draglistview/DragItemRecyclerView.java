...
class DragItemRecyclerView extends RecyclerView implements AutoScroller.AutoScrollListener {
    ...
    public interface DragItemListener {
        void onDragStarted(int itemPosition, float x, float y); //UPDATE 添加deleteView

        void onDragging(int itemPosition, float x, float y, boolean isDelete); //UPDATE 添加isDelete

        void onDragEnded(int newItemPosition);

        void onDragDelete(int newItemPosition); //ADD 拖动到删除区域后松手回调
    }
    ...
    private boolean isDelItem; //ADD 是否在删除区域
    private View mDeleteView; //ADD 删除区域视图
    ...
    private boolean shouldChangeItemPosition(int newPos) {
        ...
        Log.w("drag_item", "Alpha:0.3f");
        /*if (mDragItem.getDragItemView().getAlpha() == 0.3f) {
            Log.w("drag_item", "Alpha:0.3f");
            return false;
        }*/
    ...
    private void updateDragPositionAndScroll() {
        ...
        if (layoutManager.getOrientation() == LinearLayoutManager.VERTICAL) {
            if (!isDelItem && mDragItem.getY() > getHeight() - view.getHeight() / 2 && !lastItemReached) {
            ...
            if (!isDelItem && mDragItem.getX() > getWidth() - view.getWidth() / 2 && !lastItemReached) {
    ...
    boolean startDrag(View itemView, long itemId, float x, float y) {
        ...
        mDragItem.getDragItemView().setAlpha(1.0f);
    ...
    void onDragging(float x, float y) {
        ...
        dragDelete(x, y); //ADD 判断拖动是否在删除区域

        if (mListener != null) {
            mListener.onDragging(mDragItemPosition, x, y, isDelItem);
        }
    ...
    private void dragDelete(float x, float y) {
        if (mDeleteView != null && mDeleteView.getVisibility() == VISIBLE) {
            int[] locations = new int[2];
            mDeleteView.getLocationOnScreen(locations);
            int imageX = locations[0];// 获取组件当前位置的横坐标
            int imageY = locations[1];//获取组件当前位置的纵坐标
//            int imageY = locations[1] - DisplayUtil.dip2px(getContext(), 80); //TODO
//            int lastX = (int)x;
//            int lastY = (int)y;
//            boolean b = lastX > imageX
//                    && lastX < imageX + mDeleteView.getWidth();
//            boolean b2 = lastY < imageY
//                    && lastY > imageY - mDeleteView.getHeight();
//            if (b && b2) {
//                isDelItem = true;
//            } else {
//                isDelItem = false;
//            }
            ...
            mDragItem.getDragItemView().getLocationOnScreen(locations2);
            int lastX = locations2[0]+mDragItem.getDragItemView().getWidth()-DisplayUtil.dip2px(getContext(), 25);
            int lastY = locations2[1]+mDragItem.getDragItemView().getHeight()-DisplayUtil.dip2px(getContext(), 10);
            if (lastX > imageX && lastY > imageY) {
                isDelItem = true;
            } else {
                isDelItem = false;
            }
    ...
    private void onDragItemAnimationEnd() {
        ...
        if (mListener != null) {
            if (isDelItem) {
                mListener.onDragDelete(mDragItemPosition);
            } else {
