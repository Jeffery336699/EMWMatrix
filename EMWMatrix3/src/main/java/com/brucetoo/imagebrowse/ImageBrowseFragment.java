...
public class ImageBrowseFragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        tips = (TextView) view.findViewById(R.id.photo_tv_position);
        ...
        if (imageUrls != null && imageUrls.size() > 1) {
            String text = viewPager.getCurrentItem() + 1 + "/" + imageUrls.size();
            SpannableString spanStr = new SpannableString(text);
            ForegroundColorSpan colorSpan = new ForegroundColorSpan(Color.WHITE);
            spanStr.setSpan(colorSpan, 0, String.valueOf(viewPager.getCurrentItem() + 1).length(), Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
            RelativeSizeSpan sizeSpan = new RelativeSizeSpan(1.3F);
            spanStr.setSpan(sizeSpan, 0, String.valueOf(viewPager.getCurrentItem() + 1).length(), Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
            tips.setText(spanStr);
        } else {
