package cc.emw.mobile.contact.widget;
...
public class IndexBar extends View {
    ...
    private List<UserInfo> mSourceDatas;//Adapter的数据源
...
/*            switch (attr) {
    ...
    public IndexBar setmSourceDatas(List<UserInfo> mSourceDatas) {
    ...
    private void initSourceDatas() {
        ...
        for (int i = 0; i < size; i++) {
            UserInfo indexPinyinBean = mSourceDatas.get(i);
            StringBuilder pySb = new StringBuilder();
            String target;
            if (!indexPinyinBean.getSortLetters().equals("我关注的"))
                target = indexPinyinBean.Name;   //取出需要被拼音化的字段
            else
                target = "-1";
            if (!target.equals("-1")) {
...
//                pySb.append(Pinyin.toPinyin(target.charAt(i1)).toUpperCase());
                    if (target.charAt(i1) + "" != null && CharacterParser.getInstance().convert(target.charAt(i1) + "") != null) {
                        pySb.append(CharacterParser.getInstance().convert(target.charAt(i1) + "").toUpperCase());
                    }
                }
                indexPinyinBean.setSortLetters(pySb.toString());//设置城市名全拼音
                //以下代码设置城市拼音首字母
                String tagString = pySb.toString().substring(0, 1);
                if (tagString.matches("[A-Z]")) {//如果是A-Z字母开头
                    indexPinyinBean.setSortLetters(tagString);
                    ...
                    indexPinyinBean.setSortLetters("#");
                    if (isNeedRealIndex) {//如果需要真实的索引数据源
                ...
                indexPinyinBean.setSortLetters("我关注的");//设置城市名全拼音
                if (isNeedRealIndex) {//如果需要真实的索引数据源
                    if (!mIndexDatas.contains("")) {
//                        mIndexDatas.add("");
                    }
    ...
    private void sortData() {
        ...
        //对数据源进行排序
        Collections.sort(mSourceDatas, new Comparator<UserInfo>() {
            @Override
            public int compare(UserInfo lhs, UserInfo rhs) {
                if (lhs.getSortLetters().equals("#") || rhs.getSortLetters().equals("#")) {
                    return Integer.MAX_VALUE;
                }
                if (lhs.getSortLetters().equals("我关注的")) {
                    return -1;
                } else if (rhs.getSortLetters().equals("我关注的")) {
                    return 1;
                } else {
    ...
    private int getPosByTag(String tag) {
        ...
        for (int i = 0; i < mSourceDatas.size(); i++) {
            if (tag.equals(mSourceDatas.get(i).getSortLetters())) {
                return i;
            }
        }
        return -1;
