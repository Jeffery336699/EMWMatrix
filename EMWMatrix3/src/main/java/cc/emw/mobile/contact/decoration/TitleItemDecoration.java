package cc.emw.mobile.contact.decoration;
...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private static int COLOR_TITLE_BG = Color.parseColor("#FFFAFBFD");
    private static int COLOR_TITLE_FONT = Color.parseColor("#FF9B9B9B");
    private static int mTitleFontSize;//title字体大小
    private Context mContext;


    public TitleItemDecoration(Context context, List<UserInfo> datas) {
        super();
        mContext = context;
        mDatas = datas;
        mPaint = new Paint();
    ...
    public TitleItemDecoration setmDatas(List<UserInfo> mDatas) {
        this.mDatas = mDatas;
        return this;
    }
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0肯定要有title的
                    drawTitleArea(c, left + 20, right, child, params, position);
                } else {//其他的通过判断
                    if (null != mDatas.get(position).getSortLetters() && !mDatas.get(position).getSortLetters().equals(mDatas.get(position - 1).getSortLetters())) {
                        drawTitleAreaLine(c);
...
/*
        ...
        mPaint.getTextBounds(mDatas.get(position).getSortLetters(), 0, mDatas.get(position).getSortLetters().length(), mBounds);
        c.drawText(mDatas.get(position).getSortLetters(), child.getPaddingLeft() + DisplayUtil.dip2px(mContext, 16), child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
    }
     ...
     * @param c
     */
    private void drawTitleAreaLine(Canvas c) {
        mLinePaint.setColor(mContext.getResources().getColor(R.color.cm_divider_bg));
        c.drawLine(0, 0, DisplayUtil.getDisplayWidth(mContext), (float) 0.5, mLinePaint);
    }
    ...
    public void onDrawOver(Canvas c, final RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
        try {
            ...
            String tag = "";
            if (mDatas.size() > pos && pos >= 0)
                tag = mDatas.get(pos).getSortLetters();
//        View child = parent.getChildAt(pos);
            ...
            if (child != null) {
                ...
                if ((pos + 1) < mDatas.size()) {//防止数组越界（一般情况不会出现）
                    if (null != tag && !tag.equals(mDatas.get(pos + 1).getSortLetters())) {//当前第一个可见的Item的tag，不等于其后一个item的tag，说明悬浮的View要切换了
                ...
                c.drawText(tag, child.getPaddingLeft() + DisplayUtil.dip2px(mContext, 16),
