...
@ContentView(R.layout.activity_phone3)
public class PhoneActivity extends BaseActivity {
    ...
    private TextView mCallUserTv; //致电人文本
    @ViewInject(R.id.ll_phone_calluser_head)
    private LinearLayout mCallUserHeadLayout;//致电人头像Layout
    @ViewInject(R.id.tv_phone_calluser_num)
    private TextView mCallUserNum; //致电人数量
    @ViewInject(R.id.tv_phone_answeruser)
    private TextView mAnswerUserTv; //接听人文本
    @ViewInject(R.id.ll_phone_answeruser_head)
    private LinearLayout mAnswerUserHeadLayout;//接听人头像Layout
    @ViewInject(R.id.tv_phone_answeruser_num)
    private TextView mAnswerUserNum; //接听人数量
    @ViewInject(R.id.et_phone_telnum)
    private EditText mTelNumEt; //电话号码
    ...
    @ViewInject(R.id.ll_phone_enddate)
    ...
    @ViewInject(R.id.tv_phone_busdata)
    private TextView mBusDataTv; //业务数据
    @ViewInject(R.id.tv_phone_busname)
    private TextView mBusNameTv; //已选业务数据名称
    @ViewInject(R.id.ll_phone_busdata)
    ...
    public static final int REQUESTCODE_CANSEE = 2; //谁能看见requestCode
    public static final int REQUESTCODE_CALLUSER = 3; //选择致电人requestCode
    public static final int REQUESTCODE_ANSWERUSER = 4; //选择接听人requestCode
    public static final int REQUESTCODE_MAINUSER = 5; //选择负责人requestCode
    public static final int REQUESTCODE_FILE = 144; //选择文件requestCode
    ...
    private ArrayList<ApiEntity.UserInfo> callUserList, answerUserList, mainUserList;
    private ApiEntity.UserInfo callUser, answerUser, mainUser; //选择的致电人、接听人、负责人数据
    ...
    private void init() {
        ...
        callUser = PrefsUtil.readUserInfo();
        showCallUser(callUser);
    }
    ...
    private void setInitValue() {
        ...
        if (cInfo != null) {
            ...
            mTelNumEt.setText(cInfo.Phone);
            mRadioGroup.check(cInfo.PhoneType == 0? R.id.rb_plan_day : R.id.rb_plan_week);
        ...
        switch (v.getId()) {
            ...
            case R.id.cm_header_tv_right9:
                ...
                if (TextUtils.isEmpty(content)) {
                    ToastUtil.showToast(this, "请输入电话主题！");
                } else if (TextUtils.isEmpty(mDescEt.getText())) {
                    ToastUtil.showToast(this, "电话描述不能为空！");
                } else {
                    ...
                    if (startCalendar.getTimeInMillis() > endCalendar.getTimeInMillis()) {
                        ...
                        /*if (callUser == null) {
                            ToastUtil.showToast(this, "请选择致电人！");
                        } else*/ if (answerUser == null) {
                            ToastUtil.showToast(this, "请选择接听人！");
                        } else if (TextUtils.isEmpty(mTelNumEt.getText())) {
    ...
    @Event(value = {R.id.cm_select_ll_select, R.id.ll_phone_emergency_root, R.id.ll_phone_telnum_root,
            R.id.ll_phone_calluser_root, R.id.ll_phone_answeruser_root, R.id.ll_phone_mainuser_root,
            R.id.ll_phone_file_root, R.id.ll_phone_busdata_root, R.id.ll_repeat, R.id.ll_repeat_end, R.id.ll_remind})
    private void onSelectClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.ll_phone_file_root: //选择附件
                ...
                break;
            case R.id.ll_phone_busdata_root: //选择业务数据
            ...
            case R.id.ll_phone_emergency_root: //打开紧急程度
                ...
                break;
            /*case R.id.ll_phone_calluser_root: //选择致电人
                Intent callListIntent = new Intent(this, ContactSelectActivity.class);
                callListIntent.putExtra(ContactSelectActivity.EXTRA_SELECT_TYPE, ContactSelectActivity.RADIO_SELECT);
                callListIntent.putExtra("select_user", callUser);
...
//                callListIntent.putExtra("select_list", callUserList);
                ...
                break;*/
            case R.id.ll_phone_answeruser_root: //选择接听人
                Intent answerListIntent = new Intent(this, ContactSelectActivity.class);
                answerListIntent.putExtra(ContactSelectActivity.EXTRA_SELECT_TYPE, ContactSelectActivity.RADIO_SELECT);
                answerListIntent.putExtra("select_user", answerUser);
//                answerListIntent.putExtra(ContactSelectActivity.EXTRA_SELECT_TYPE, ContactSelectActivity.MULTI_SELECT);
//                answerListIntent.putExtra("select_list", answerUserList);
                answerListIntent.putExtra("start_anim", false);
                int[] location5 = new int[2];
                v.getLocationOnScreen(location5);
                answerListIntent.putExtra("click_pos_y", location5[1]);
                startActivityForResult(answerListIntent, REQUESTCODE_ANSWERUSER);
                break;
...
//                mainListIntent.putExtra("select_list", mainUserList);
                ...
                mainListIntent.putExtra("click_pos_y", location6[1]);
                startActivityForResult(mainListIntent, REQUESTCODE_MAINUSER);
                break;
            case R.id.ll_phone_telnum_root:
                mTelNumEt.requestFocus();
                HelpUtil.showSoftInput(this, mTelNumEt);
                break;
        }
    }
        ...
        if (resultCode == Activity.RESULT_OK) {
            switch (requestCode) {
                ...
                case REQUESTCODE_CANSEE: //选择人员回传的数据
                    ...
                    break;
                case REQUESTCODE_CALLUSER: //选择致电人回传的数据
//                    callUserList = (ArrayList<ApiEntity.UserInfo>) data.getSerializableExtra("select_list");
//                    showCallUser(callUserList);
                    callUser = (ApiEntity.UserInfo) data.getSerializableExtra("select_user");
                    showCallUser(callUser);
                    break;
                case REQUESTCODE_ANSWERUSER: //选择接听人回传的数据
//                    answerUserList = (ArrayList<ApiEntity.UserInfo>) data.getSerializableExtra("select_list");
//                    showAnswerUser(answerUserList);
                    answerUser = (ApiEntity.UserInfo) data.getSerializableExtra("select_user");
                    showAnswerUser(answerUser);
                    break;
...
//            mCallUserTv.setVisibility(View.VISIBLE);
    ...
    private void showAnswerUser(ApiEntity.UserInfo userInfo) {
//        mAnswerUserTv.setVisibility(View.GONE);
...
//            mAnswerUserTv.setVisibility(View.VISIBLE);
            mAnswerUserNum.setVisibility(View.GONE);
        }
    }
...
//            mCallUserNum.setVisibility(View.VISIBLE);
...
//            mAnswerUserNum.setVisibility(View.VISIBLE);
...
//            mMainUserTv.setVisibility(View.VISIBLE);
    ...
    private void showAnswerUser(int uid) {
//        mAnswerUserTv.setVisibility(View.GONE);
        mAnswerUserNum.setVisibility(View.GONE);
        mAnswerUserHeadLayout.setVisibility(View.VISIBLE);
        mAnswerUserHeadLayout.removeAllViews();
...
//            mMainUserTv.setVisibility(View.VISIBLE);
    ...
    private CalendarInfo getCInfo() {
        ...
        cInfo.Launcher = callUser.ID;
        cInfo.T_Launcher = callUser.Name;
        cInfo.Receiver = answerUser.ID;
        cInfo.T_Receiver = answerUser.Name;
        ...
        cInfo.IsJobSync = 1;
        cInfo.Color = 2;
        cInfo.Type = ApiEnum.UserNoteAddTypes.Phone;
        cInfo.NoteType = ApiEnum.UserNoteAddTypes.Phone;
        RadioButton selectRb = (RadioButton) findViewById(mRadioGroup.getCheckedRadioButtonId());
        cInfo.PhoneType = Integer.valueOf(selectRb.getTag().toString());
        cInfo.Title = mNameEt.getText().toString();
        cInfo.Remark = mDescEt.getText().toString();
        cInfo.Phone = mTelNumEt.getText().toString();
        ...
        API.TalkerAPI.AddCalendar(cInfo, new RequestCallback<ApiEntity.APIResult>(
            ...
            public void onParseSuccess(ApiEntity.APIResult respInfo) {
                ...
                if (respInfo != null && respInfo.State == 1) {
                    ...
                    if (isEdit) {
                        ToastUtil.showToast(PhoneActivity.this, "保存成功！", R.drawable.tishi_ico_gougou);
                        setResult(Activity.RESULT_OK);
                    } else {
                        ToastUtil.showToast(PhoneActivity.this, getString(R.string.groupcreate_success), R.drawable.tishi_ico_gougou);
                    }
                    finish();
                } else {
                    if (isEdit) {
                        Toast.makeText(PhoneActivity.this, "保存失败！", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(PhoneActivity.this, getString(R.string.groupcreate_error), Toast.LENGTH_SHORT).show();
                    }
                }
            }
        });
                    ...
                    if (position != 0) {
                        ...
                        switch (position) {
                            ...
                            case 6:
                                ...
                                break;
                            case 7:
                                Intent intent = new Intent(PhoneActivity.this, FreeRepeatActivity.class);
                                intent.putExtra("start_anim", false);
