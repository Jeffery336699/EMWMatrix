...
@ContentView(R.layout.activity_video_capture2)
public class VideoCaptureActivity2 extends BaseActivity {
    ...
    @ViewInject(R.id.itv_but_camera)
    private IconTextView mItvButCamera;
    @ViewInject(R.id.itv_chat_camera_but_image)
    private IconTextView mChatCameraChange;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mItvButCamera.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                stopTimer();
                resetTimer();
                commonVideo.stopVideo();
                if(commonVideo.getCamera() !=null) {
                    commonVideo.getCamera().stopPreview();//停掉原来摄像头的预览
                    commonVideo.getCamera().release();//释放资源
                    commonVideo.setCamera(null);//取消原来摄像头
                    SystemClock.sleep(500);
                }
                SystemClock.sleep(500);
                Intent intent=new Intent(VideoCaptureActivity2.this, TakePhoteActivity.class);
        ...
        mBtn.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        isShow=true;
                        file = new File(EMWApplication.tempPath + generateFileName());
        ...
        mChatCameraChange.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                commonVideo.change();
            }
