...
public class QqFilter extends EmoticonFilter {
    ...
    public static final Pattern QQ_RANGE = Pattern.compile("\\[face:[0-9]+\\.png\\]");  // \[[a-zA-Z0-9\u4e00-\u9fa5]+\]
    ...
    public static Spannable spannableFilter(Context context, Spannable spannable, CharSequence text, int fontSize, EmojiDisplayListener emojiDisplayListener) {
        ...
        if (m != null) {
            while (m.find()) {
                String key = m.group();
                int icon = DefQqEmoticons.sQqEmoticonHashMap.get(key) == null ? 0 : DefQqEmoticons.sQqEmoticonHashMap.get(key);
