...
public class BaseFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        injected = true;
        mRoot = x.view().inject(this, inflater, container);
        return mRoot;
    }
    ...
    public void startActivity(Intent intent) {
        if (!intent.hasExtra("start_anim") || intent.getBooleanExtra("start_anim", false)) {
            ...
            if (Build.VERSION.SDK_INT >= 21) {
                super.startActivity(intent, ActivityOptions.makeSceneTransitionAnimation(getActivity()).toBundle());
            } else {
                super.startActivity(intent);
            }
        }
    ...
    public void startActivityForResult(Intent intent, int requestCode) {
        if (!intent.hasExtra("start_anim") || intent.getBooleanExtra("start_anim", false)) {
            ...
            if (Build.VERSION.SDK_INT >= 21) {
                super.startActivityForResult(intent, requestCode, ActivityOptions.makeSceneTransitionAnimation(getActivity()).toBundle());
            } else {
                super.startActivityForResult(intent, requestCode);
            }
        }
