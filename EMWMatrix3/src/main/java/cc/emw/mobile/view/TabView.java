    ...
    public TabView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < N; i++) {
            switch (typedArray.getIndex(i)) {
            	case R.styleable.TabView_text_visibility:
            		isTextVisibility = typedArray.getBoolean(i, true);
            		break;
...
//                Log.v("zgy","=============position="+position+",====positionOffset="+positionOffset) ;
                ...
                if (positionOffset > 0) {
                    ...
                    Log.d(TAG,"onPageScrolled()-> position: " + position + " count: " +  mViewPager.getChildCount());
    ...
    private void initItem() {
        for (int i = 0; i < mChildSize; i++) {
			if (i == 2) { // 中间圆形菜单留的区域
				View tempItem = new View(getContext());
				LayoutParams params = new LayoutParams(0, 1, 1);
				tempItem.setLayoutParams(params);
				addView(tempItem);
			}
            ...
            tabItem.setTextVisibility(isTextVisibility);
    ...
    public void setTabItemDeSelect(){
        for (TabItem tabItem : mTabItems) {
            tabItem.setTabAlpha(0);
        }
    }
