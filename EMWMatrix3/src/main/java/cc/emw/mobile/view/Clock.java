...
public class Clock extends View {
	...
	private float mMinutes, mOldM;
	private float mHour, mOldH;
	private float mSeconds, mOldS;
...
//		canvas.rotate(mHour / 12.0f * 360.0f, x, y);
		if ((int)mOldH < (int)mHour) {
			canvas.rotate(mOldH / 12.0f * 360.0f, x, y);
			mOldH++;
			postInvalidateDelayed(300);
		} else if ((int)mOldH > (int)mHour){
			canvas.rotate(mOldH / 12.0f * 360.0f, x, y);
			mOldH--;
			postInvalidateDelayed(300);
		} else {
...
//		canvas.rotate(mMinutes / 60.0f * 360.0f, x, y);
		if ((int)mOldM < (int)mMinutes) {
			canvas.rotate(mOldM / 60.0f * 360.0f, x, y);
			mOldM = mOldM + 5;
			if (mOldM > mMinutes)
				mOldM = mMinutes;
			postInvalidateDelayed(300);
		} else if ((int)mOldM > (int)mMinutes){
			canvas.rotate(mOldM / 60.0f * 360.0f, x, y);
			mOldM = mOldM - 5;
			if (mOldM < mMinutes)
				mOldM = mMinutes;
	...
	public void onTimeChanged(Time timeObject) {
		...
		Log.d("clock", "onTimeChanged   H:"+(int)mHour+",M:"+(int)mMinutes);
	}
