...
public class NsRefreshLayout extends FrameLayout {
    ...
    public boolean hasNoMoreData = true;
    ...
    protected void onFinishInflate() {
        ...
        Log.d("NSUP", mContentView.toString());
        setupViews();
    }
    ...
    public void removeCircleProgress() {
        footerView.removeCircleProgress();
    }

    public void addCircleProgress() {
        footerView.addCircleProgress();
    }
...
//                    actionDetermined = true;
                    ...
                    } else if (dy < 0 && !canChildScrollDown() && mPullLoadEnable && Math.abs(dy) > Math.abs(dx)) {
                        mCurrentAction = ACTION_PULL_UP_LOAD_MORE;
                        Log.d("NSUP", "ACTION_PULL_UP_LOAD_MORE");
                    } else {
                        mCurrentAction = -1;
                    }
                    Log.d("NSUP", "mCurrentAction:" + mCurrentAction);
...
//                }
        ...
        if (!canChildScrollUp() && mCurrentAction == ACTION_PULL_DOWN_REFRESH &&
            ...
            if (lp.height < loadingViewOverHeight) {
                ...
                if (hasNoMoreData) {
                    ...
                    footerView.setLoadText(getContext().getString(R.string.release_to_load_hasnodata));
                }
            }
    ...
    private UpScrollFlagListener mListener;

    public void setUpScrollFlagListener(UpScrollFlagListener listener) {
        mListener = listener;
    }

    public interface UpScrollFlagListener {
        boolean onUp();
    }
    ...
    public boolean canChildScrollDown() {
        ...
        if (Build.VERSION.SDK_INT < 14) {
//            Log.d("NSUP","版本："+Build.VERSION.SDK_INT);
            if (mContentView instanceof AbsListView) {
                final AbsListView absListView = (AbsListView) mContentView;
                ...
                Log.d("NSUP", "1:" + mContentView.getScrollY());
                return ViewCompat.canScrollVertically(mContentView, 1) || mContentView.getScrollY() > 0;

            }
        } else {
            Log.d("NSUP", "2:" + ViewCompat.canScrollVertically(mContentView, 1));
            return ViewCompat.canScrollVertically(mContentView, 1);
